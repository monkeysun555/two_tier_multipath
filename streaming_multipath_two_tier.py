...
class streaming(object):
	...
	def run(self, network_trace, yaw_trace, pitch_trace, video_trace, rate_cut):
		while self.video_seg_index_BL < VIDEO_LEN or \
			...
			previous_time = self.video_fetching(network_trace, rate_cut, yaw_trace, pitch_trace)
			# return
			## SHOULD MODIFY
			if not self.downloadedPartialVideo and self.video_version >= 1:
	...
	def video_fetching(self, network_trace, rate_cut, yaw_trace, pitch_trace):
				...
				if self.buffer_size_EL < ET_MAX_PRED:
					...
					self.EVR_EL_Recordset.append([self.video_seg_index_EL, self.video_version, network_trace[self.network_seg_index], \
					   ...
					   self.pitch_predict_value_quan, pitch_trace[self.video_seg_index_EL*VIDEO_FPS-VIDEO_FPS/2],\
					    VP_SPAN_YAW, VP_SPAN_PITCH, temprol_eff, RETRAN_UTILI_INIT])
		...
		if np.floor(self.video_download_timestamp) != np.floor(temp_video_download_timestamp):
			...
			if self.retransmit_available and self.do_correct:
				self.correct(yaw_trace, pitch_trace, rate_cut)

		return temp_video_download_timestamp


	## Check 5G availability
	def retransmit_available(self):
		availability = np.random.uniform(0,1)
		if availability >= DISABLE_RATE:
			return True
		else:
			return False
	...
	def correct(self,yaw_trace, pitch_trace, rate_cut):
		if self.correctness_using == 0:
			correct_time = np.floor(self.video_download_timestamp) - RTT_2
			...
			if self.correct_ptr >= len(self.EVR_EL_Recordset):
				...
				if next_EL[0] > correct_index:
					...
					if distance <= 15:
						...
						downloading_5G = retran_data_size/retran_bandwidth
...
def display(record_info, EVR_BL_Recordset, EVR_EL_Recordset, rate_cut, yaw_trace, pitch_trace, network_trace, bw_info):
	...
	plt.plot(display_result[0], display_result[2],'b-', label='Enhancement Tier Buffer Length')
	...
	# plt.ylim(-1.5, 2.0)
	h = plt.figure(5)
	plt.plot(display_result[7], display_result[3],'b-', label='Predict Viewport (horizontal)')
	...
	for i in range(0,len(EVR_EL_Recordset)):
		...
		for j in range(0, VIDEO_FPS):
			...
			sum_eff += eff
		sum_eff /= VIDEO_FPS
		display_bitrate[EVR_EL_Recordset[i][0]] += sum_eff*rate_cut[EVR_EL_Recordset[i][1]]*EVR_EL_Recordset[i][9]
		receive_bitrate[EVR_EL_Recordset[i][0]] += rate_cut[EVR_EL_Recordset[i][1]]
		extra_cost[EVR_EL_Recordset[i][0]] = EVR_EL_Recordset[i][10]/8
	...
	if IS_CORRECT:
		q = plt.figure(7)
		plt.bar(range(1,VIDEO_LEN+1), extra_cost, label='Retransmit Data')
		# plt.plot(range(1,VIDEO_LEN+1), receive_bitrate, 'r-', label='Received Video Bitrate')
		plt.legend(loc='best')
		plt.title('Extra Data Cost')
		plt.xlabel('Second')
		plt.ylabel('MB')
		plt.axis([0, 600, 0, max(extra_cost)+1])
	...
	if IS_CORRECT:
		q.show()
	# f.savefig('BT_&_ET_Buffer_Length.eps', format='eps', dpi=1000)
	...
	# p.savefig('Bandwidth_Predict_&_Real_Trance.eps', format='eps', dpi=1000)
