...
Q_REF_BL = 10
Q_REF_EL = 1
Q_REF_BL_ADAPTIVE = 15
Q_REF_EL_ADAPTIVE = 1
ET_MAX_PRED = Q_REF_EL + 1
ET_MAX_PRED_ADAPTIVE = Q_REF_EL_ADAPTIVE + 1
...
class streaming(object):
	...
	def control(self, rate_cut, sniff_BW, do_correction):
		...
		if self.buffer_size_BL < self.bl_ref and self.video_seg_index_BL < VIDEO_LEN :
			...
			if len(self.buffer_size_history) != 0:
				...
				for index in range(1, min(PI_RANGE+1, len(self.buffer_size_history)+1)):
					...
					# else:
					u_i +=  KI  * (self.buffer_size_history[-index][1] - self.buffer_size_history[-index][3])
			u = u_i + u_p
			########################
...
def main():
	...
	rate_cut_bt, rate_cut_et = new_rate_determine(network_cdf)
	video_trace = loadVideoTrace(rate_cut_bt, rate_cut_et)
	# print(video_trace)
	# return
	streaming_sim = streaming()
	streaming_sim.run(network_trace, yaw_trace, pitch_trace, video_trace, rate_cut_bt, rate_cut_et, network_trace_aux)
...
def loadVideoTrace(rate_cut_bt, rate_cut_et):
	video_trace_bt0 = rate_cut_bt[0] * np.ones(VIDEO_LEN)
	video_trace_bt1 = rate_cut_bt[1] * np.ones(VIDEO_LEN)
	video_trace_et11 = (rate_cut_et[0][0] - rate_cut_bt[0]) * np.ones(VIDEO_LEN)
	video_trace_et12 = (rate_cut_et[0][1] - rate_cut_bt[0]) * np.ones(VIDEO_LEN)
	video_trace_et13 = (rate_cut_et[0][2] - rate_cut_bt[0]) * np.ones(VIDEO_LEN)
	video_trace_et21 = (rate_cut_et[1][0] - rate_cut_bt[1]) * np.ones(VIDEO_LEN)
	video_trace_et22 = (rate_cut_et[1][1] - rate_cut_bt[1]) * np.ones(VIDEO_LEN)
	video_trace_et23 = (rate_cut_et[1][2] - rate_cut_bt[1]) * np.ones(VIDEO_LEN)
	return [[video_trace_bt0, video_trace_et11, video_trace_et11, video_trace_et13], \
			[video_trace_bt1, video_trace_et21, video_trace_et22, video_trace_et23]]
...
def new_rate_determine(network_cdf):
	...
	# 		if cut_idx >= len(cut_percent): 
	# 			break
	rate_cut_et = [[]] * ADAPTIVE_NUM
	rate_cut_bt = [60.0, 400.0]
	rate_cut_et[0] = [400.0, 500.0, 600.0]
	rate_cut_et[1] = [500.0, 550.0, 600.0]
	# rate_cut_et[2] = 500
	# if IS_DEBUGGING: 
		# print('Rate0 = %f, Rate1 = %f, Rate2 = %f, Rate3 = %f' % (rate_cut[0],rate_cut[1],rate_cut[2],rate_cut[3]))
	return rate_cut_bt, rate_cut_et
