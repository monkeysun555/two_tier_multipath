...
if AVE_RATIO == 0.85:
	# if IS_AGAIN:
	# 	RATE_DISCOUNT = 0.835/0.85 ##float(0.75/0.85)
	# else:
	RATE_DISCOUNT = 1.0
else:
	RATE_DISCOUNT = 1.0
...
class streaming(object):
	...
	def run(self, network_trace, yaw_trace, pitch_trace, video_trace, rate_cut, network_trace_aux):
		while self.video_seg_index_BL < VIDEO_LEN or \
			...
			if not self.downloadedPartialVideo:
				...
				if IS_AGAIN:
					if self.video_version >= 0:
						...
						print("enter from main run!!!!!")
						temp_time = self.video_download_timestamp
						downlading = self.correct(yaw_trace, pitch_trace, rate_cut, network_trace)
						if np.floor(self.video_download_timestamp) == np.floor(temp_time) and downlading:
							# print("Correction!!!!!!!!")
			...
			# print("Before Fetching:", self.video_version,self.video_segment_index, self.video_download_timestamp, self.buffer_size_EL)
		...
		print("Network segment idnex:", self.network_seg_index)
		print("EL record length:", len(self.EVR_EL_Recordset))
		print("Finished Time:", self.video_download_timestamp)
		print("ET buffer empty:", self.GBUFFER_EL_EMPTY_COUNT)
		print("BT buffer empty:", self.GBUFFER_BL_EMPTY_COUNT)
		print("Extra Time:", self.addition_time)
		print("Extra Data:", self.addition_data)
		print("Useful Data:", self.addition_useful_data)
	...
	def control(self, rate_cut, sniff_BW, do_correction):
		...
		# if not self.downloadedPartialVideo:
		# print("control buffer:", self.buffer_size_BL, self.video_download_timestamp)
	...
	def video_fetching(self, network_trace, rate_cut, yaw_trace, pitch_trace, network_trace_aux):
		...
		if network_trace[self.network_seg_index]*self.remaining_time >= self.video_segment : #or self.buffer_size_EL > ET_MAX_PRED:
			if self.video_version == 0:
				...
				if self.buffer_size_EL < ET_MAX_PRED:
					...
					# self.video_segment = 0
					# self.video_version = 0
					# print("1111111..1.........1112q''''''''")
					# print(self.video_download_timestamp)
					if self.video_download_timestamp < (np.floor(self.video_download_timestamp) + AGAIN_THRES):
						if self.video_download_timestamp < (np.floor(self.video_download_timestamp) + AGAIN_THRES - IDEL_STEP):
							# print("plus step")
							# print(self.video_download_timestamp, self.remaining_time)
							self.remaining_time = max(0.0, self.remaining_time - IDEL_STEP)
							if self.remaining_time == 0.0:
								self.remaining_time = 1.0
							self.downloadedPartialVideo = 0
							self.video_download_timestamp += IDEL_STEP
							self.video_segment = 0
							self.video_version = 0
							record_bw = 0
							# print(self.video_download_timestamp, self.remaining_time)
							...
							second_temp = self.video_download_timestamp
							self.video_segment = 0
							self.video_version = 0

							if IS_AGAIN:
								# print("enter from fetching!!!!!")
								downlading = self.correct(yaw_trace, pitch_trace, rate_cut, network_trace)
								...
								if np.floor(self.video_download_timestamp) == np.floor(temp_video_download_timestamp):
									...
									recording_EL = 0
									self.buffer_size_EL = max(0.0, self.buffer_size_EL - (second_temp - temp_video_download_timestamp))
									# print("-12-3-12-31-23-1-23-1, pass 1s")
									return temp_video_download_timestamp, recording_EL
					else:
						print("Jump directly!!!!!!!", self.video_download_timestamp)
						self.remaining_time = 1.0
						self.downloadedPartialVideo = 0
						self.video_download_timestamp = np.ceil(self.video_download_timestamp+0.000001)
						self.video_segment = 0
						self.video_version = 0
			...
			if not self.downloadedPartialVideo and record_bw:
			...
			self.remaining_time = 1.0 - self.request_pre
			self.downloadedPartialVideo = 1
			self.request_pre = 0
			# if self.video_version > 0:
			# 	if self.video_seg_index_EL > np.floor(temp_video_download_timestamp) + 1:
			# 		is_same_el_index = 0
		...
		if np.floor(self.video_download_timestamp) != np.floor(temp_video_download_timestamp):
			...
			# self.buffer_size_BL = np.round(self.buffer_size_BL)
			# self.buffer_size_EL = np.round(self.buffer_size_EL)
			...
			print("buffer length:", self.video_download_timestamp, self.buffer_size_BL, self.buffer_size_EL, self.remaining_time)
	...
	def correct(self,yaw_trace, pitch_trace, rate_cut, network_trace_aux):
		...
		print("enter time:", self.video_download_timestamp)
			...
			if self.correct_ptr >= len(self.EVR_EL_Recordset):
				...
				if next_EL[0] > correct_index:
					...
					if distance <= 15:
						...
						print("retran data:", retran_data_size)
...
def new_rate_determine(network_cdf):
	...
	# ##  75% average , re1 = 0.75*re2  re3=1.25re2
	# if VER == 11:
	# 	##  3 9
	# 	rate_cut[0] = 27.70
	# 	rate_cut[1] = 392.30
	...
	for i in range(len(rate_cut)):
		rate_cut[i] *= RATE_DISCOUNT
...
def gamma_rate_determine(network_trace):
	...
	rate0 = average* AVE_RATIO * RBE_RATIO
...
def display(record_info, EVR_BL_Recordset, EVR_EL_Recordset, rate_cut, yaw_trace, pitch_trace, network_trace, bw_info, buffer_size_history, network_trace_aux, half_sec_network_trace):
	...
	global FIGURE_NUM
	f = plt.figure(FIGURE_NUM,figsize=(20,5))
	FIGURE_NUM +=1
	# np.array(buffer_size_history.T[0])
	...
	plt.plot(range(1,len(buffer_size_history)+5), np.append(np.array(buffer_size_history).T[0], \
		[np.array(buffer_size_history).T[0][-1]-1, np.array(buffer_size_history).T[0][-1]-2, \
		np.array(buffer_size_history).T[0][-1]-3,np.array(buffer_size_history).T[0][-1]-4]),'r*-', markersize = 6, markeredgewidth = 0.5, \
			linewidth = 2, markeredgecolor = 'red',  label='BT Buffer Length')
	plt.plot(range(1,len(buffer_size_history)+5), np.append(np.array(buffer_size_history).T[1], \
		[np.array(buffer_size_history).T[1][-1], max(np.array(buffer_size_history).T[1][-1]-1,0.0), max(np.array(buffer_size_history).T[1][-1]-2,0.0),\
			max(np.array(buffer_size_history).T[1][-1]-3,0.0)]),'bo-', markersize = 2, markeredgewidth = 0.5, \
			linewidth = 2, markeredgecolor = 'blue',  label='ET Buffer Length')
	# plt.plot(len(buffer_size_history), np.array(buffer_size_history).T[0][-1]-1,'r+-', markersize = 6, markeredgewidth = 0.5, \
	# 	linewidth = 2, markeredgecolor = 'red')
	# plt.plot(len(buffer_size_history)+1, np.array(buffer_size_history).T[1][-1]-1,'bo-', markersize = 2, markeredgewidth = 0.5, \
	# 	linewidth = 2, markeredgecolor = 'blue')
	...
	plt.axis([0, 300, 0, 20])
	## Change file name
	# f.savefig('two_tier_buff_21.eps', format='eps', dpi=1000, figsize=(30, 10))

	# plt.ylim(-1.5, 2.0)
	# global FIGURE_NUM
	h = plt.figure(FIGURE_NUM,figsize=(20,5))
	FIGURE_NUM +=1
	up_imaginary = [180.0]*VIDEO_LEN
	low_imaginary = [-180.0]*VIDEO_LEN
	zero_line = [0.0]*VIDEO_LEN
	for i in range(len(display_result[4])):
		if display_result[4][i] < 0:
			display_result[4][i] += 360
	plt.plot(display_result[7], display_result[3],'b-', label='Predict Viewport (horizontal central)')
	plt.plot(display_result[7], display_result[4],color='cornflowerblue', label='FoV Direction',linewidth=2.5)
	# print(len(np.arange(0,VIDEO_LEN,1.0/float(VIDEO_FPS))))
	# print(len(yaw_trace[:VIDEO_LEN*VIDEO_FPS]))
	# plt.plot(np.arange(0,VIDEO_LEN, 1.0/VIDEO_FPS), [x - 180 for x in yaw_trace[:VIDEO_LEN*VIDEO_FPS]],color='cornflowerblue', label='FoV Trace 2',linewidth=2.5)
	...
	plt.tick_params(axis='both', which='major', labelsize=30)
	plt.tick_params(axis='both', which='minor', labelsize=30)
	plt.axis([0, VIDEO_LEN, 0 , 400])
	plt.xticks(np.arange(0, VIDEO_LEN+1, 50))
	plt.yticks(np.arange(0, 400+1, 90))
	plt.gcf().subplots_adjust(bottom=0.20, left=0.085, right=0.97)	
	# plt.grid(linestyle='dashed', axis='y', linewidth=1.5,color='gray')
	# h.savefig('fov_trace_2.eps', format='eps', dpi=1000,figsize=(30, 9))
	...
	# g.savefig('two_tier_rate_21.eps', format='eps', dpi=1000, figsize=(30, 10))
